...
'''
'''
this template uses a get_percent_gc_content (module or script) and a function we created in class to aid comprehension.
name your actual unit test script "test_<get_percent_gc_content WE'RE TESTING>.py"
'''

# 1. get the function to test or import the module to test.
from get_percent_gc_content import get_percent_gc_content

# 2. create a class into which we pass the unit tests we want to perform.
class Test_get_percent_gc_content(unittest.TestCase):
